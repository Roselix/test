...
exports.commands = {
	away: function (target, room, user) {
		...
		if (user.can('lock', null, room)) this.add("|raw|-- <font color='" + Galaxy.hashColor(user.userid) + "'><strong>" + Chat.escapeHTML(user.name) + "</strong></font> is now " + target + ".");
	...
	back: function (target, room, user) {
		...
		if (statusIdx < 0) {
			user.isAway = false;
			if (user.can('lock', null, room)) this.add("|raw|-- <font color='" + Galaxy.hashColor(user.userid) + "'><strong>" + Chat.escapeHTML(user.name) + "</strong></font> is no longer away.");
			return false;
		}
		...
		user.updateIdentity();
		user.isAway = false;
		if (user.can('lock', null, room)) this.add("|raw|-- <font color='" + Galaxy.hashColor(user.userid) + "'><strong>" + Chat.escapeHTML(newName) + "</strong></font> is no longer " + status + ".");
	},
