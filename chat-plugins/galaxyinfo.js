/*
The following commands were coded specifically for Galaxy
Made by Insist
feelscool
...
exports.commands = {
	...
	opensource: function (target, room, user) {
		if (!this.runBroadcast()) return;
		this.sendReplyBox(
			"Galaxy's Github's:<br />" +
			"- Language: JavaScript (Node.js)<br />" +
			"- <a href=\"https://github.com/DeathlyPlays/Exiled\">Galaxy Server Code</a><br />"
		);
	},
	opensourcehelp: [
		"/opensource - Links to Galaxy's source code repository.",
	...
	forums: function (target, room, user) {
		if (!this.runBroadcast()) return;
		this.sendReplyBox("<a href=\"http://galaxy.psim.us.forumcommunity.net/\">Galaxy Forum</a>");
	},/*

	'!suggestions': true,
	suggestions: function (target, room, user) {
		if (!this.runBroadcast()) return;
		this.sendReplyBox("<a href=\"http://Galaxyps.boards.net/board/3/suggestions\">Make a suggestion for Galaxy</a>");
	},
	...
	discord: function (target, room, user) {
		if (!this.runBroadcast()) return;
		this.sendReplyBox("<a href=\"https://discord.gg/u7H5BUY\">The Official Galaxy Discord</a>");
	},

	'!facebook': true,
	facebook: function (target, room, user) {
		if (!this.runBroadcast()) return;
		this.sendReplyBox("<a href=\"https://www.facebook.com/Galaxyserver/\">The Official Galaxy Facebook Page</a>");
	},
	...
	bugs: function (target, room, user) {
		...
		if (room && room.battle) {
			...
			this.sendReplyBox(
				"Have a replay showcasing a bug on Galaxy?<br />" +
				"- <a href=\"http://Galaxyps.boards.net/thread/5/bug-reports/\">Bug Reports</a>"
			);
		}
	},
	...
	rules: function (target, room, user) {
		if (!target) {
			...
			this.sendReplyBox("Please follow the rules:<br />" +
				...
				"- <a href=\"http://Galaxyps.boards.net/thread/4/Galaxy-rules-regulations\">" + (room && room.rulesLink ? "Global rules" : "Rules") + "</a>");
			return;
		}
		if (!room) {
	...
	credits: function (target, room, user) {
		let popup = "|html|" + "<font size=5 color=#F7189F><u><b>Galaxy Credits:</b></u></font><br />" +
			"<br />" +
			"<u><b>Server Maintainers:</u></b><br />" +
			"- " + Galaxy.nameColor('Dentice', true) + " (Owner, Sysadmin, Development)<br />" +
			"- " + Galaxy.nameColor('Hzard', true) + " (Owner, Sysadmin, Development)<br />" +
			"<br />" +
			"<u><b>Major Contributors:</b></u><br />" +
			"- " + Galaxy.nameColor('Dentice', true) + " (Developments)<br />" +
			"- " + Galaxy.nameColor('Hzard', true) + " (Developments)<br />" +
			"- " + Galaxy.nameColor('Not Vinz', true) + " (Developments)<br />" +
			/*"- " + Galaxy.nameColor('Back At My Day', true) + " (Developments)<br />" +*/
			"<br />" +
			"<u><b>Retired Staff:</b></u><br />" +
			"- " + Galaxy.nameColor('Hzard', true) + " (Former Owner, Development, CSS Developments)<br />" +
			"- " + Galaxy.nameColor('Nko Bellic', true) + " (Developments)<br />" +
			"<br />" +
