...
function generateNews(user) {
	...
	Db('news').keys().forEach(announcement => {
		...
		newsDisplay.push(`<h4>${announcement}</h4>${newsData[1]}<br /><br />—${Galaxy.nameColor(newsData[0], true)} <small>on ${newsData[2]}</small>`);
	});
	return newsDisplay;
}
...
Galaxy.showNews = function (userid, user) {
...
exports.commands = {
	...
	serverannouncements: {
		...
		request: function (target, room, user) {
			...
			if (!target) return this.sendReply("/news request [message] - Requests a news announcement from Galaxy Staff.");
			if (target.length < 1) return this.sendReply("/news request [message] - Requests a news announcement from Galaxy Staff.");
			...
			Rooms('staff').update();
			Galaxy.messageSeniorStaff('A news requested has been submitted by ' + user.name + '. ID: ' + newsId + ' Request Message: ' + target.trim());
			return this.sendReply("Your request has been sent to Galaxy global authorities..");
		},
	},
