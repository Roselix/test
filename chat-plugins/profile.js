...
Profile.prototype.avatar = function () {
	if (this.isOnline) {
		if (typeof this.image === 'string') return img('https://hultragalaxy-flareninja29.c9users.io/avatars/' + this.image);
	...
	for (let name in Config.customAvatars) {
		if (this.username === name) {
			...
			return img('https://hultragalaxy-flareninja29.c9users.io/avatars/' + Config.customAvatars[name]);
		}
	}
...
Profile.prototype.seen = function (timeAgo) {
	if (this.isOnline) return label('Last Seen') + '<font style="color:#006400;"><b>Currently Online</b></font>';
...
Profile.prototype.dev = function (person) {
	...
	if (person === 'insist') return '<font color="#33CCFF"><b>(Owner)</b></font> <font color="#FF0000"><b>(VIP)</b></font>';
	return '<font color="#FF0000"><b>(VIP)</b></font>';
};
...
exports.commands = {
	...
	badge: function (target, room, user) {
		...
		if (!acceptable.includes(toId(parts[0]))) return this.parse('/badgehelp');
		switch (parts[0]) {
		case 'add':
			let id = toId(parts[1]);
			let name = parts[1];
		...
		case 'set':
			...
			break;
		case 'take':
			let usertarget = toId(parts[1]);
			let hasbadges = Db('badges').get(usertarget);
			let deletebadge = toId(parts[2]);
	...
	badgehelp: function (target, room, user) {
		let display = '';
		display += '<div class="infobox-limited"><center><b>Galaxy Badge Plugin By Amber, Insist, and Execute.</b></center>';
	...
	profilehelp: function (target, room, user) {
		let display = '';
		display += '<div class="infobox-limited"><center><b>Galaxy Profiles Created by Insist, Volco, and Execute</b></center>';
