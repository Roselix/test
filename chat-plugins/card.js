...
const shop = [
	['XY-Base', 'Ottieni quattro carte dalla prima espansione rilasciata nella serie di Pokemon XY.', 10],
	['XY-Flashfire', 'Ottieni quattro carte dalla espansione Fuoco Infernale della serie XY.', 12],
	['XY-Phantom Forces', 'Ottieni quattro carte dalla espansione Forze Spettrali della serie XY.', 12],
	['XY-Roaring Skies', 'Ottieni quattro carte dalla espansione Furie Volanti della serie XY.', 13],
	['XY-Ancient Origins', 'Ottieni quattro carte dalla espansione Antiche Origini della serie XY.', 14],
	['XY-Primal Clash', 'Ottieni quattro carte dalla espansione Scontro Primordiale della serie XY.', 15],
	['XY-Furious Fists', 'Ottieni quattro carte dalla espansione Colpi Furiosi della serie XY.', 10],
	['GXplosion', 'Ottieni quattro carte GX da questa fantastica espansione!', 65],
	['4GenNostalgia', 'Ottieni quattro carte direttamente dalla quarta generazione!', 65],
    ['Alola-Lovers', 'Ottieni quattro carte direttamente da Alola! Incluse in questa espansione le rarissime GX Ghost!', 65],
	['Fairy', 'Ottieni quattro carte di tipo Folletto!', 25],
	['LC-Pack', 'Ottieni quattro carte della tier Little Cup!', 5],
	['Gen3', 'Ottieni quattro carte di Terza Generazione!', 30],
];
let packShop = ['XY-Base', 'XY-Flashfire', 'XY-Phantom Forces','XY-Roaring Skies', 'XY-Ancient Origins', 'XY-Primal Clash', 'XY-Furious Fists', 'GXplosion','4GenNostalgia', 'Alola-Lovers', 'Fairy', 'LC-Pack', 'Gen3', 'Galaxy', 'Double Crisis', 'Water', 'Fire', 'Fighting', 'Fairy', 'Dragon', 'Colorless', 'Psychic', 'Lightning', 'Darkness', 'Grass', 'UU-Pack', 'Uber-Pack', 'PU-Pack', 'NU-Pack', 'RU-Pack', 'LC-Pack', 'BL-Pack', 'BL2-Pack', 'BL3-Pack', 'Gen1', 'Gen2', 'Gen3', 'Gen4', 'Gen5', 'Gen6', 'Metal', 'Trainer', 'Supporter', 'Item', 'Stadium', 'EX-Pack', 'Legendary', 'Full', 'Event'];
const tourCardRarity = ['No Card', 'Common', 'Uncommon', 'Rare', 'Epic', 'Legendary', 'Mythic', 'Deluxe'];
const cardRarity = ['Common', 'Uncommon', 'Rare', 'Epic', 'Legendary', 'Mythic', 'Deluxe', 'Godlike', 'InYourDreams'];
let cleanShop = [];
let cleanCard = [];
...
Galaxy.tourCard = function (tourSize, userid) {
	if (tourSize > 12) tourSize = 12;
...
exports.commands = {
	...
	buypack: function (target, room, user) {
		...
		if (packId !== 'xybase' && packId !== 'xyflashfire' && packId !== 'xyphantomforces' && packId !== 'xyroaringskies' && packId !== 'xyancientorigins' && packId !== 'xyprimalclash' && packId !== 'xyfuriousfists' && packId !== 'gxplosion' && packId !== '4gennostalgia' && packId !== 'alolalovers' && packId !== 'fairy' && packId !== 'lcpack' && packId !== 'gen3') return self.sendReply("This pack is not currently in circulation.  Please use /packshop to see the current packs.");
	...
	openpack: function (target, room, user) {
		...
		for (let i = 0; i < 4; i++) {
			...
			this.sendReplyBox(Galaxy.nameColor(user.name, true) + ' got <font color="' + colors[cards[card].rarity] + '">' + cards[card].rarity + '</font> ' +
	...
	showcase: function (target, room, user) {
		...
		if (!cards.length || userid === "constructor") return this.sendReplyBox(Galaxy.nameColor(userid, false) + " has no cards.");
		...
		this.sendReplyBox('<div style="max-height: 300px; overflow-y: scroll;">' + cardsMapping.join('') + '</div><br><center><b>' + Galaxy.nameColor(userid, false) + ' has ' + cards.length + ' cards and ' + getPointTotal(userid) + ' points.</b></center>');
	},
	...
	cardladder: function (target, room, user) {
		...
		let keys = Object.keys(Db('points').object()).map(function (name) {
			return {name: Galaxy.nameColor(name, false), points: getPointTotal(name)};
		});
	...
	searchcard: function (target, room, user) {
		...
		const categories = {
			Rarity: ['Common', 'Uncommon', 'Rare', 'Epic', 'Legendary', 'Mythic', 'Deluxe','Godlike', 'InYourDreams'], // rarities
			Packs: ['XY-Base', 'XY-Flashfire', 'XY-Phantom Forces', 'XY-Roaring Skies', 'XY-AncienT Origins', 'XY-Primal Clash', 'XY-Furious Fists', 'GXplosion', '4GenNostalgia', 'Alola-Lovers', 'EX-Pack', 'Trainer', 'XY-Promo'],
	...
	tradecard: function (target, room, user) {
		...
		if (Users.get(targetUser)) Users.get(targetUser).send("|pm|~Galaxy Server|" + targetUser + "|/raw <div class=\"broadcast-green\">" + Galaxy.nameColor(user.name, true) + " has initiated a trade with you.  Click <button name=\"send\" value=\"/trades last\">here</button> or use <b>/trades</b> to view your pending trade requests.</div>");
		user.send("|pm|~Galaxy Server|" + user.userid + "|/raw <div class=\"broadcast-green\">Your trade with " + Galaxy.nameColor(targetUser, true) + " has been initiated.  Click <button name=\"send\" value=\"/trades last\">here</button> or use <b>/trades</b> to view your pending trade requests.</div>");
	},
	...
	viewcardtrades: function (target, room, user) {
		...
		let userSideDisplay = '<center>' + Galaxy.nameColor(user.userid, true) + '<br />' + cardImage + "<br />" + cardRarityPoints + '</center>';
		...
		let targetSideDisplay = "<center>" + (displayTrade.from !== user.userid ? Galaxy.nameColor(displayTrade.from, true) : Galaxy.nameColor(displayTrade.to, true)) + '<br />' + cardImage + "<br />" + cardRarityPoints + "</center>";
	...
	tradeaction: function (target, room, user) {
		...
		case 'accept':
			...
			if (targetUsers[0]) {
				targetUsers[0].popup("|html|" + backButton + "<center>Your trade with " + Galaxy.nameColor(trade.from, true) + " has gone through." +
				"<br /><button name=\"send\" value=\"/cs card, " + trade.fromExchange + "\">View Traded Card</button></center>"); // show card
			}
			if (targetUsers[1]) {
				targetUsers[1].popup("|html|<center>Your trade with " + Galaxy.nameColor(trade.to, true) + " has gone through." +
